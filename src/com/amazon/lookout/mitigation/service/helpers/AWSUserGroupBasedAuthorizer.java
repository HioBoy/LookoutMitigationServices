...
public class AWSUserGroupBasedAuthorizer {
    ...
    private static final String ROUTE53_SERVICE_NAME = ServiceName.Route53;
    
    public static final String ROUTE53_POP_ROUTER_CLIENT = generateClientName(ROUTE53_SERVICE_NAME, DeviceName.POP_ROUTER.name(), false);
    public static final String ROUTE53_POP_ROUTER_READONLY_CLIENT = generateClientName(ROUTE53_SERVICE_NAME, DeviceName.POP_ROUTER.name(), true);
    public static final String ROUTE53_ANY_DEVICE_READONLY_CLIENT = generateClientName(ROUTE53_SERVICE_NAME, DeviceName.ANY_DEVICE.name(), true);
    
    private static final Log LOG = LogFactory.getLog(AWSUserGroupBasedAuthorizer.class);
    
    @ConstructorProperties({"groupIdToClient", "operationToClient", "operationsToUseWriteCreds"})
    public AWSUserGroupBasedAuthorizer(@Nonnull Map<String, String> groupIdToClientMap, @Nonnull Map<String, List<String>> operationToClientsMap,
    ...
    /**
     * Checks if the client is authorized to perform operation on the serviceName+deviceName. 
     * All operations are permitted for any serviceName+deviceName combinations for calls from LookoutMitigationServiceExplorer
     */
    private boolean isClientAuthorizedForServiceAndDevice(Identity identity, String serviceName, String deviceName, String operation) {
        if (!isValidClient(identity) || serviceName == null || deviceName == null || operation == null || (!operationToClientsMap.containsKey(operation))) {
        ...
        for (String group : groups) {
            ...
            // If the client belongs to the right serviceName+deviceName combination or is LookoutMitigationServiceExplorer then authorize request
            if (client.equals(expectedClientWithWritePermissions) || 
                ...
                // Now check if the client is authorized for the operation if this is not a LOOKOUT_MITIGATION_SERVICE_EXPLORER
                if (client.equals(LOOKOUT_MITIGATION_SERVICE_EXPLORER) || authorizedClients.contains(client)) {
    ...
    /**
     * Checks if the client is authorized to perform operation on the serviceName for any device.
     */
    ...
    /**
     * Sets the authorized flag to indicate that we have authorized this service call.
     */
