...
public class ListBlackWatchMitigationsActivity extends Activity {
    private static final Log LOG = LogFactory.getLog(ListBlackWatchMitigationsActivity.class);
    ...
    public @NonNull ListBlackWatchMitigationsResponse enact(@NonNull ListBlackWatchMitigationsRequest request) {
        TSDMetrics tsdMetrics = new TSDMetrics(getMetrics(), "ListBlackWatchMitigations.enact");
        
        String requestId = getRequestId().toString();
        boolean requestSuccessfullyProcessed = true;
        
        try {            
            LOG.info(String.format("ListBlackWatchMitigationsActivity called with RequestId: %s and request: %s.", requestId, ReflectionToStringBuilder.toString(request)));
            ...
            List<BlackWatchMitigationDefinition> listOfBlackWatchMitigationDefinition = blackwatchMitigationInfoHandler.getBlackWatchMitigations(mitigationId, resourceId, resourceType, ownerARN, maxNumberOfEntriesToReturn, tsdMetrics);
            ...
            String msg = String.format(ActivityHelper.BAD_REQUEST_EXCEPTION_MESSAGE_FORMAT, requestId, "ListBlackWatchMitigationsActivity", ex.getMessage());
            LOG.warn(msg + " for request: " + ReflectionToStringBuilder.toString(request), ex);
            tsdMetrics.addCount(ActivityHelper.EXCEPTION_COUNT_METRIC_PREFIX + ListBlackWatchMitigationsExceptions.BadRequest.name(), 1);
            throw new BadRequest400(msg);
        } catch (Exception internalError) {
            String msg = "Internal error in ListBlackWatchMitigationsActivity for requestId: " + requestId + ", reason: " + internalError.getMessage();
            ...
            tsdMetrics.addCount(ActivityHelper.EXCEPTION_COUNT_METRIC_PREFIX + ListBlackWatchMitigationsExceptions.InternalError.name(), 1);
